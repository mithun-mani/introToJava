...
public class Sorts {
    ...
    public static <T extends Comparable<? super T>> void heapify(T[] arr, int arrLength, int i) {

        int max = i;
        int l = 2 * i + 1;
        int r = 2 * i + 2;

        if (l < arrLength && (arr[l].compareTo(arr[max])) > 0){
            max = l;
        }

        if (r < arrLength && (arr[r].compareTo(arr[max])) > 0) {
            max = r;
        }
